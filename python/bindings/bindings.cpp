...
PYBIND11_MODULE(_superkmeans, m) {
    m.doc() = "A Super fast K-Means library for High-Dimensional vectors on CPUs (x86, ARM) and GPUs";

    py::class_<skmeans::SuperKMeansConfig>(m, "SuperKMeansConfig",
        "Configuration parameters for SuperKMeans clustering.")
        .def(py::init<>(), "Default constructor")

        .def_readwrite("iters", &skmeans::SuperKMeansConfig::iters,
            "Number of k-means iterations (default: 10)")
        .def_readwrite("sampling_fraction", &skmeans::SuperKMeansConfig::sampling_fraction,
            "Fraction of data to sample, 0.0 to 1.0 (default: 0.3)")
        .def_readwrite("max_points_per_cluster", &skmeans::SuperKMeansConfig::max_points_per_cluster,
            "Maximum number of points per cluster to sample (default: 256)")
        .def_readwrite("n_threads", &skmeans::SuperKMeansConfig::n_threads,
            "Number of CPU threads, 0 = use all available (default: 0)")
        .def_readwrite("seed", &skmeans::SuperKMeansConfig::seed,
            "Random seed for reproducibility (default: 42)")
        .def_readwrite("use_blas_only", &skmeans::SuperKMeansConfig::use_blas_only,
            "Use GEMM-only computation without pruning (default: False)")

        .def_readwrite("tol", &skmeans::SuperKMeansConfig::tol,
            "Tolerance for shift-based early termination (default: 1e-4)")
        .def_readwrite("recall_tol", &skmeans::SuperKMeansConfig::recall_tol,
            "Tolerance for recall-based early termination (default: 0.005)")
        .def_readwrite("early_termination", &skmeans::SuperKMeansConfig::early_termination,
            "Whether to stop early on convergence (default: True)")
        .def_readwrite("sample_queries", &skmeans::SuperKMeansConfig::sample_queries,
            "Whether to sample queries from data (default: False)")
        .def_readwrite("objective_k", &skmeans::SuperKMeansConfig::objective_k,
            "Number of nearest neighbors for recall computation (default: 100)")

        .def_readwrite("verbose", &skmeans::SuperKMeansConfig::verbose,
            "Whether to print progress information (default: False)")
        .def_readwrite("angular", &skmeans::SuperKMeansConfig::angular,
    ...
    py::class_<skmeans::SuperKMeansIterationStats>(m, "SuperKMeansIterationStats",
        "Statistics for a single iteration of SuperKMeans clustering.")
        .def(py::init<>(), "Default constructor")
        .def_readonly("iteration", &skmeans::SuperKMeansIterationStats::iteration,
            "Iteration number (1-indexed)")
        .def_readonly("objective", &skmeans::SuperKMeansIterationStats::objective,
            "Total clustering cost (WCSS)")
        .def_readonly("shift", &skmeans::SuperKMeansIterationStats::shift,
            "Average squared centroid shift from previous iteration")
        .def_readonly("split", &skmeans::SuperKMeansIterationStats::split,
            "Number of clusters that were split (empty cluster handling)")
        .def_readonly("recall", &skmeans::SuperKMeansIterationStats::recall,
            "Recall@k value (0.0 to 1.0)")
        .def_readonly("not_pruned_pct", &skmeans::SuperKMeansIterationStats::not_pruned_pct,
            "Percentage of vectors not pruned (-1.0 if not applicable)")
        .def_readonly("partial_d", &skmeans::SuperKMeansIterationStats::partial_d,
            "Number of dimensions used for partial distance (d')")
        .def_readonly("is_gemm_only", &skmeans::SuperKMeansIterationStats::is_gemm_only,
    ...
    py::class_<skmeans::SuperKMeans<skmeans::Quantization::f32, skmeans::DistanceFunction::l2>>(
        m, "SuperKMeans",
        "SuperKMeans clustering"
    )
        .def(py::init([](size_t n_clusters, size_t dimensionality,
                         const skmeans::SuperKMeansConfig& config) {
            return new skmeans::SuperKMeans<skmeans::Quantization::f32,
                                           skmeans::DistanceFunction::l2>(
                n_clusters, dimensionality, config
            );
        }), py::arg("n_clusters"), py::arg("dimensionality"), py::arg("config"),
        ...
        .def(py::init([](size_t n_clusters, size_t dimensionality) {
            return new skmeans::SuperKMeans<skmeans::Quantization::f32,
                                           skmeans::DistanceFunction::l2>(
                n_clusters, dimensionality
            );
        }), py::arg("n_clusters"), py::arg("dimensionality"),
            "Initialize SuperKMeans with default configuration.\n\n"
            "Args:\n"
            "    n_clusters: Number of clusters to create\n"
            "    dimensionality: Number of dimensions in the data")

        .def("train", [](skmeans::SuperKMeans<skmeans::Quantization::f32,
            ...
            std::memcpy(result_ptr, centroids_vec.data(),
                       centroids_vec.size() * sizeof(float));

            return result;
        }, py::arg("data"), py::arg("queries") = py::none(), py::arg("n_queries") = 0,
        ...
        .def("assign", [](skmeans::SuperKMeans<skmeans::Quantization::f32,
                                                skmeans::DistanceFunction::l2>& self,
            ...
            auto assignments_vec = self.Assign(vectors_ptr, centroids_ptr,
                                              n_vectors, n_centroids);
            ...
            // TODO(@lkuffo, high): I am not a fan of this memcpy. Can we do better?
            std::memcpy(result_ptr, assignments_vec.data(),
                       assignments_vec.size() * sizeof(uint32_t));

            return result;
        }, py::arg("vectors"), py::arg("centroids"),
        ...
        .def("get_n_clusters", &skmeans::SuperKMeans<skmeans::Quantization::f32,
                                                      skmeans::DistanceFunction::l2>::GetNClusters,
            "Get the number of clusters.\n\n"
            "Returns:\n"
            "    Number of clusters")

        .def("is_trained", &skmeans::SuperKMeans<skmeans::Quantization::f32,
                                                  skmeans::DistanceFunction::l2>::IsTrained,
            "Check whether the model has been trained.\n\n"
            "Returns:\n"
            "    True if trained, False otherwise")

        .def_readonly("iteration_stats",
        ...
        .def("__repr__", [](const skmeans::SuperKMeans<skmeans::Quantization::f32,
                                                        skmeans::DistanceFunction::l2>& self) {
