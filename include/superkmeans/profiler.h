...
public:
    ...
    void Stop(const std::string& name) {
        ...
        if (it != timers_.end() && it->second.running) {
            it->second.accum_time_ns += 
                std::chrono::duration_cast<std::chrono::nanoseconds>(end - it->second.start).count();
    ...
    void Print(std::ostream& os = std::cout) const {
        ...
        os << std::left << std::setw(35) << "TOTAL" 
           << std::right << std::setw(10) << (total_ns / 1e9) << "s\n";
        os << "=======================================\n";
    }
        ...
        for (const auto& name : top_level) {
            ...
            if (it != groups.end()) {
                ...
                for (const auto& child : children) {
                    ...
                    double child_pct = total_ns > 0 ? (child_data.accum_time_ns * 100.0 / total_ns) : 0.0;
                    std::string short_name = "  - " + child.substr(name.length() + 1);
                    
                    os << std::left << std::setw(40) << short_name
                       << std::right << std::setw(10) << child_secs << "s"
